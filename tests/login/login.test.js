...
describe('Login', function () {
    ...
    it('[CT01] - Validar a exibição e funcionalidade do botão "Login".', async function () {
        await menuScreen.clickBtnLogin()
        assert.equal(await menuScreen.getAttributeBtnLogin(), 'Login')
    })

    it('[CT02] - Verificar mensagens de erro ao deixar os campos de e-mail e senha em branco na tela "Login".', async function () {
    ...
    it('[CT03] - Validar as labels da subtela "Login" dentro da tela "Login".', async function () {
        await menuScreen.clickBtnLogin()
        assert.equal(await menuScreen.getAttributeBtnLogin(), 'Login')
    ...
    it('[CT04] - Validar o preenchimento dos campos de e-mail e senha na tela "Login".', async function () {
         await menuScreen.clickBtnLogin()
         await loginScreen.addValueInpEmailLogin('MarlonAmorim@gmail.com')
         await loginScreen.addValueInpPasswordLogin('Marlon123teste')
         await loginScreen.clickBtnloginForm()
         await loginScreen.clickBtnOk()
    })

    it('[CT05] - Verificar as ações e os resultados esperados da subtela "Sign Up" na tela "Login".', async function () {
         assert.equal(await loginScreen.getTextLblSubTitleSignUp(), 'Sign up')
         await loginScreen.clickBtnSystemConfirm()
         await loginScreen.clickBtnSubTitleSignUp()
         await loginScreen.addValueInpEmailSignUp('MarlonAmorim@gmail.com')
         await loginScreen.addValueInpPasswordSignUp('Marlon123teste')
         await loginScreen.addValueInpConfirmPasswordSignUp('Marlon123teste')
         await loginScreen.clickBtnloginSignUp()
         await loginScreen.clickBtnOk()
    })
